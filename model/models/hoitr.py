...
from ..utils.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       ...
                       tensor_list_from_nested_tensor)

from .backbone import build_backbone, rebuild_backbone
from .hoi_matcher import rebuild_matcher as build_hoi_matcher
from .transformer import build_transformer, rebuild_transformer

# change this value to suit your dataset
num_ho = 112
...
class HOITR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_actions, num_queries, aux_loss=False):
        ...
        self.human_cls_embed = nn.Linear(hidden_dim, num_ho + 1)
    ...
    def get_featureandpos(self, samples: NestedTensor):
        """返回由backbone提取的特征和位置编码"""
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples)
        return features, pos
    
    def get_hoitr_embeds(self, samples: NestedTensor):
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples)
    ...
    def get_ab_hoitr_embeds(self, samples: NestedTensor):
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples)
        ...
        zero_pos = torch.zeros_like(pos[-1])

        hs = self.transformer(self.input_proj(src), mask, self.query_embed.weight, zero_pos)[0]
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, num_actions, matcher, weight_dict, eos_coef, losses):
        ...
        human_empty_weight = torch.ones(num_ho + 1)
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        human_target_classes = torch.full(human_src_logits.shape[:2], num_ho,
...
def build_HOITR(config):
    ...
    backbone = rebuild_backbone(args)

    transformer = rebuild_transformer(args)
