...
from .places365 import build_place
...
class Tag2Text(nn.Module):
    ...
    def __init__(self,
                 ...
                 tag_list=f'{CONFIG_PATH}/data/HOIP2A_tag_list.txt',
                 stage='eval'):
        super().__init__()
        # create tokenzier
        self.tokenizer = init_tokenizer(text_encoder_type)
        # HOI2A employ encoder-decoder architecture for image-tag-text generation: hoi encoder and image-tag-text decoder
        # create hoi encoder
        model, criterion = build_HOITR(config = '/home/bit118/wangyuhan/Index_HOIP2A/HOI2A/configs/hoitr_config.json')
        place_model, place_classes = build_place(config = '/home/bit118/wangyuhan/Index_HOIP2A/HOI2A/configs/places_config.json')
        self.tagging_head = model
        self.tagging_place = place_model
        self.place_classes = place_classes
        self.load_and_freeze_hoitr_model('/home/bit118/wangyuhan/Index_HOIP2A/pretrain_models/HOTR/resnet50_hotr_image224.pth')
    ...
    # 将hoitr的dict结果转换为tensor
    def dict2tensor(self, samples, dict, image_ori_size, hoi_th=0.5, human_th=0.5, object_th=0.5, max_to_viz=10, HOI_res=False):
        ...
        # print("HOI Result:", final_hoi_result_list)
    ...
    def gen_tensor(self, res_list, num_labels):
        ...
        # 只保留 h_name, o_name, i_name 对应的标签号
        tags_classes = {}
        tags_bboxes = {}
        for hoi in hoi_list:
    ...
    def gen_place(self, place_img):
        
        logit = self.tagging_place.forward(place_img)
        h_x = F.softmax(logit, 1).data.squeeze()
        probs, idx = h_x.sort(0, True)
        cls_place = probs[0]
        place = (self.place_classes[idx[0]]).replace('_', ' ')

        return place, cls_place
    ...
    def forward(self, samples, caption, tag, image_ori_size):
        ...
        # 消融实验，零pos
        # trans_image_embeds = self.tagging_head.get_ab_hoitr_embeds(samples).permute(1, 3, 0, 2)
        # trans_image_embeds = trans_image_embeds.flatten(2).permute(0, 2, 1)
        # image_embeds  = self.imageto1024(trans_image_embeds)
        # image_embeds = image_embeds.to(torch.float32)
    ...
    def generate(self,
        ...
        # 消融实验，零pos
        # trans_image_embeds = self.tagging_head.get_ab_hoitr_embeds(hoi_samples).permute(1, 3, 0, 2)
        # trans_image_embeds = trans_image_embeds.flatten(2).permute(0, 2, 1)
        # image_embeds  = self.imageto1024(trans_image_embeds)
        # image_embeds = image_embeds.to(torch.float32)
        ...
        if tag_input == None:
            ...
            # place, place_cls = self.gen_place(place_samples)
